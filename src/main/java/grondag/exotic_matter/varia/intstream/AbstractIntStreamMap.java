...
public abstract class AbstractIntStreamMap<V extends AbstractStreamCursor<V>> extends AbstractIntStreamCollection<V>
{
    ...
    /**
     * Call before adding an element to set.
     * Will call {@link #onExpand(int, int, int, int)} if rehash is needed.
    ...
    /**
     * Adds key/value currently in writer to the map.
     * Returns true if key was not already present.<p>
     * 
     ...
     * Returns true if entry was found.<p>
     *  
     * Reader will not be moved and will reflect deletion
     * if reader happens to be on the deleted entry.p>
     * 
