...
public class QuadBakery
{
    ...
    public static BakedQuad createBakedQuad(IPolygon raw, boolean forceItemFormat)
    {
        ...
        applyTextureRotation(raw.getRotation(), uvData);
...
//                ? net.minecraft.client.renderer.vertex.DefaultVertexFormats.ITEM
    ...
    public static BakedQuad createBakedQuad(int layerIndex, IPaintedPoly raw, boolean forceItemFormat)
    {
        ...
        for(int v = 0; v < 4; v++)
        {
            final IPaintedVertex vertex = raw.getVertex(v);
            if(vertex.glow(layerIndex) != 0)
            { 
                // round to nearest 0-15
                int g = (vertex.glow(layerIndex)  + 9) / 17;
                glowBits |= (g << (v * 4));
            }
            
            uvData[v][0] = vertex.u(layerIndex);
            uvData[v][1] = vertex.v(layerIndex);
